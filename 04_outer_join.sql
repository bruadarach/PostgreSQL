...
SELECT
  c.name AS city,
  l.name AS language -- 1. From cities (alias as c)
FROM cities as c -- 2. Join to languages (alias as l)
INNER JOIN languages AS l -- 3. Match on country code
  ON c.country_code = l.code -- 4. Where c.name like Hyderabad
...
WHERE
  year = 2010 -- Order by life_exp
ORDER BY
  life_exp -- Limit to 5 records
LIMIT
  5;
